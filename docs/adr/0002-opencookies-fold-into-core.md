# ADR 0002 — OpenCookies folds into `@policystack/core`, not a standalone consent package
...
The consent runtime depends on `@policystack/core`'s `JurisdictionId` /
...
it does not weaken `@policystack/core`'s `sideEffects:false` contract. Subpath
...
### 1. Consent core → `@policystack/core/consent`

OpenCookies `@opencookies/core` is folded into `packages/core/src/consent/**`
and published as the `@policystack/core` subpaths `./consent`,
...
### 3. Scope stays `@policystack/*`

Consent lands as `@policystack/*`, consistent with the eight existing packages
...
`@policystack/{react,vue,svelte}` expose `./policy` (renderers) and `./consent`
(banner adapter) as independent entries; the bare `.` export is removed.
`@policystack/{solid,angular}` are new consent-only packages exposing `./consent`;
`@policystack/scripts` is the standalone vendor-tag-loader package. Core keeps a
...
freeze (plan Decision 5). Living inside `@policystack/core` does not extend the
...
Originally: `@policystack/angular` ships TypeScript source (no `ng-packagr`, no
...
- **Breaking (pre-1.0, expected):** every `@policystack/{react,vue,svelte}`
...
- The SDK bridge moved `@policystack/sdk/opencookies` → `@policystack/sdk/consent`
  and now type-imports `@policystack/core/consent`; the external
...
- PS-17 shrinks: `@policystack/core` is already in the fixed changeset group, so
  only the new `@policystack/{solid,scripts}` packages need adding.
- Downstream tickets re-path `@policystack/consent` → `@policystack/core/consent`
