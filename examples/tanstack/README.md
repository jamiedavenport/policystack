...
# TanStack Start Example — the canonical OpenPolicy reference

This is **the** reference app for OpenPolicy: one TanStack Start (React +
...
scanner regression net (the `openPolicy()` plugin scans this source to
...
- Configuring the `openPolicy()` plugin in `vite.config.ts` alongside
  TanStack Start.
...
src/policystack.gen.ts         ← generated by openPolicy()'s source scan
...
- [OpenPolicy](https://openpolicy.sh)
