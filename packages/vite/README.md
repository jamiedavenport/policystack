...
`policystack.gen.ts` is meant to be committed: it carries the scanned values
_and_ the type augmentation that makes `defineConfig` demand a legal-context
entry per scanned key, so committing it keeps both live in CI without running
Vite first.

Because it is committed, it lands under whatever formatter you run — and the
plugin's output style will not match every formatter. So the plugin rewrites it
only when the scan actually changed. Each file's header carries a digest of the
scanned content:
...
On the next build the plugin compares that digest against the current scan and
skips the write when they match, whatever the file has been reformatted to
since. Reformat it freely; builds will not fight you. A missing or unreadable
header always rewrites, so deleting the file (or the comment) regenerates it.
