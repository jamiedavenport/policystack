...
- 6fd9262: Add an optional `locale` prop to `<PrivacyPolicy />` and `<CookiePolicy />`. Overrides `config.locale` at render time, so a single config can render multiple locales side-by-side:
...
- f50ecd1: Make `PrivacyPolicy` and `CookiePolicy` overridable end-to-end so they work on React Native (Expo).
