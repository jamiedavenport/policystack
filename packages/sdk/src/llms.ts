...
GPC, …) in the optional \`consent\` block, then pass the whole config to the
single provider — \`<PolicyStack config={config}>\` from
\`@policystack/react/provider\` (and the same-named provider in
\`@policystack/vue\`/\`@policystack/solid\`). It supplies both the policy
context and the consent store from one context; \`<PrivacyPolicy>\`/
\`<CookiePolicy>\` (from \`@policystack/react/policy\`) and \`useConsent\`/
\`ConsentGate\` (from \`@policystack/react/consent\`) all read from it. The
cookies → consent-categories derivation happens inside \`createConsentStore\`
(\`@policystack/core/consent\`) when given the whole config — there is no
separate user-facing conversion step.
